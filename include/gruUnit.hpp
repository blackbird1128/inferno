...
public:
	...
	float Compute()
	{
		...
		for (auto i = 0; i < Enter.size(); i++)
		{
			UpdateGate.push_back(Sigmoid(Enter[i] * EnterUpdateWeights[i] + previousEnter[i] * PreviousEnterUpdateWeight[i]));
			ResetGate.push_back(Sigmoid(Enter[i] * EnterResetWeight[i] + previousEnter[i] * PreviousEnterResetWeight[i]));
