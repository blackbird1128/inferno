...
namespace Inf
{
	...
	public:
		...
		void backPropagation(std::vector<float > target, std::pair<std::vector<float>, std::vector<float> > & error1and2)
		{
			...
			{
				auto error = Inf::quadraticLoss(target[i], Layer[i].FinalMemory[0]); // fixme
				...
				err10.push_back(err8[i] * Inf::tanhDerivative(Layer[i].CurrentMemoryContent[0]));
				err11.push_back(err9[i] * Inf::SigmoidPrime(Layer[i].UpdateGate[0]));
				...
				err18.push_back(err17[i] * Inf::SigmoidPrime(Layer[i].ResetGate[0]));
